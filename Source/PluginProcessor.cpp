...
double YAMSAudioProcessor::getTailLengthSeconds() const { return 0.0; }
...
int YAMSAudioProcessor::getCurrentProgram() { return 0; }

void YAMSAudioProcessor::setCurrentProgram(int index) {}

const juce::String YAMSAudioProcessor::getProgramName(int index) { return {}; }

void YAMSAudioProcessor::changeProgramName(int index,
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool YAMSAudioProcessor::isBusesLayoutSupported(
...
void YAMSAudioProcessor::processBlock(juce::AudioBuffer<float> &buffer,
  ...
  for (int channel = 0; channel < totalNumInputChannels; ++channel) {
    for (int n = 0; n < buffer.getNumSamples(); n++) {
      ...
      input =
          preEQSaturationStage.processSample(input, saturation * .5, channel);
      input = toneSection.processSample(input, channel, tone);
      input =
          postEQSaturationStage.processSample(input, saturation * .5, channel);
...
void YAMSAudioProcessor::setStateInformation(const void *data,
                                             int sizeInBytes) {
...
YAMSAudioProcessor::createParams() {
  std::vector<std::unique_ptr<RangedAudioParameter>> params;

  // Adding input slider to params vector
  params.push_back(std::make_unique<AudioParameterFloat>(
      "INPUTVOLUME", "Input Volume", -12.f, 12.f, 0.f));

  params.push_back(std::make_unique<AudioParameterFloat>(
      "SATURATION", "Saturation", 0.f, 3.f, 0.f));

  params.push_back(
      std::make_unique<AudioParameterFloat>("TONE", "Tone", -12.f, 12.f, 0.f));

  params.push_back(std::make_unique<AudioParameterFloat>(
      "THRESHOLD", "Threshold", -20.f, 6.1f, 6.1f));

  params.push_back(
      std::make_unique<AudioParameterFloat>("LIMIT", "Limit", -3.f, 0.f, 0.f));

  params.push_back(std::make_unique<AudioParameterFloat>(
      "OUTPUTVOLUME", "Output Volume", -12.f, 12.f, 0.f));
