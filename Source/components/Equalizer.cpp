...
float Equalizer::processSample(float input, float channel, float subGain,
...
void Equalizer::setFs(int Fs) { this->Fs = Fs; }
...
void Equalizer::setLowFreq(float freq) { lowPeakFilter.setFreq(freq); }

void Equalizer::setMidFreq(float freq) { midPeakFilter.setFreq(freq); }

void Equalizer::setHighFreq(float freq) { highPeakFilter.setFreq(freq); }
